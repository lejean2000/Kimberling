...
cOrth[a_, b_, c_] := Module[{ac, bc, pHA, pHB}, ac = lineEqPt[a, c]; 
      bc = lineEqPt[b, c]; pHA = footPerpendicular[a, bc]; 
      pHB = footPerpendicular[b, ac]; lineIntersection[lineEqPt[a, pHA], 
       lineEqPt[b, pHB]]]
 
cAubertLine[a_, b_, c_, d_] := Module[{z, ab, cd}, ab = lineEqPt[a, b]; 
      cd = lineEqPt[c, d]; z = lineIntersection[ab, cd]; 
      lineEqPt[cOrth[z, b, c], cOrth[z, a, d]]]
 
cConicCenter[{{a_, h_, g_}, {h_, b_, f_}, {g_, f_, c_}}] = 
    {(-(b*g) + f*h)/(a*b - h^2), (-(a*f) + g*h)/(a*b - h^2)}
 
implicitTangent[func_, {x0_, y0_}] := Module[{f, k, x, y}, 
